...
class HDF5Cache():
    ...
    def cache_images(self, num_cache, images, labels):
        filename = './datasets/cached/' + self.generate_cached_name(num_cache*self.cache_size)
    ...
    def get_item(self, idx, dataset):
        ...
        if self.cache_handler == None or self.cache_handler.filename != filename:
            self.load_cache(idx)
        return self.cache_handler['lr'][idx % self.cache_size], self.cache_handler['hr'][idx % self.cache_size]
